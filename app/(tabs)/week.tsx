...
import { groupByDay } from '@/utils/forecastAggregation';

export default function WeekScreen() {
  const { points, loading, refreshing, error, refresh } = useWeather(
    SLEIPNER_LOCATION.latitude,
    SLEIPNER_LOCATION.longitude
  );
  const days = groupByDay(points, 7);

  return (
    <SafeAreaView className="flex-1 bg-offshore-bg" edges={['top', 'bottom']}>
      <View className="px-5 pt-4">
        <Text className="text-3xl font-bold text-offshore-text">7 dager</Text>
        <Text className="mt-1 text-sm text-offshore-textMuted">
          Landingssannsynlighet og enkel værmelding for helikopterflyging
        </Text>
      </View>
